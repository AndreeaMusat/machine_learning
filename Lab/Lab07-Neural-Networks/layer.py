...
class Layer:
    ...
    def backward(self, inputs, output_errors):
        ...
        # print('output errors shape', output_errors.shape)

        self.g_biases = output_errors

        # TODO (2.b.ii)
        # Compute the gradients w.r.t. the weights (self.g_weights)
        self.g_weights = np.dot(self.g_biases, inputs.T)

        # TODO (2.b.iii)
        # Compute and return the gradients w.r.t the inputs of this layer
        return np.dot(self.weights.T, output_errors) * self.f(inputs, derivate=True)
        ######################################################
...
def test_linear_layer():
    ...
    assert (l.g_biases.shape == gbias_target.shape), "Wrong bias size"
    assert close_enough(l.g_biases, gbias_target), "Wrong bias values"
    print("     OK")
